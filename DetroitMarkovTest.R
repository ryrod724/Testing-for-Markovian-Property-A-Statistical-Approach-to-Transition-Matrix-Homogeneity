...
weather.data<- read.csv("DetroitWeatherData.csv", header=TRUE, sep=",")

DT<- weather.data$Weather

tomorrow.all<- ifelse(DT=="ms", "A", ifelse(DT %in% c("mc","pc", "c", "t"),"B", ifelse(DT %in% c("f"),"C","D")))
#A - sun, B - cloud, C - fog, D - rain/snow

tomorrow.all<-tomorrow.all[2:367] #07/19/2023-07/18/2024
